...
public class AllowedHostsPolicyTests
{
    ...
    // 結果として許可リストは [" * "] のまま残り、どの Host とも一致しないので
    // <b>すべて 400</b> になる（実測。HostFilteringShortCircuitTests が固定）。
    // ここを true にすると「一致しない項目をワイルドカードと呼ぶ」ことになり、規則と食い違う
    [InlineData("  *  ", false)]
    [InlineData("incident.example.com; * ", false)]
    // 空白だけの値も同じ（項目が 1 件残るので既定の ["*"] へは落ちない）。
    // <b>これと直上の "  *  " は、生きた項目が 1 つも無いので全ホストが落ちる。</b>
    // 部分的に落ちるのは生きた項目と混ざった "incident.example.com; * " のほう ——
    // 判定の理屈は 3 つとも同じだが、運用上の症状は残りの項目しだいで変わる
    [InlineData("   ", false)]
