...
///   <item>同じ版が<b>生の補間文字列</b>(<c>$$"""…{{x}}…"""</c>)もふつうの補間文字列として
...
public class ModelStateKeyPrefixMatchTests
{
    ...
    /// 区切りの規則がまったく別で(穴の区切りは <c>$</c> の数だけ並んだ波括弧、終端は開始と
    ...
    /// 走査されない(本文として中身ごと潰されるため)。ここを正しく見るには穴の区切りを
    /// <c>$</c> の数から決める専用の走査が要るが、このリポジトリに生の補間文字列は
    /// <b>1 つも無い</b>ので、実在しない事情のために分岐を増やさない
    /// (CLAUDE.md §6「将来を見越した過度な抽象化を避ける」)。<b>使い始めた時点で、
    /// 穴を走査する処理を足すこと</b>——取りこぼしは静かなので、この注記が唯一の手がかりになる。</para>
    /// </summary>
