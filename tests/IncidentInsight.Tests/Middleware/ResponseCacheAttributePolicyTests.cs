...
public class ResponseCacheAttributePolicyTests
{
    ...
    public void CodeScan_FindsTheEndOfEveryKindOfStringLiteral()
    {
        ...
        // <b>その行の中で閉じないフェンスは、行末まで飲み込まない。</b>
        // 飲み込むと、同じ行の後ろに置かれた @*…*@ がコメントとして落ちず、
        // §5 どおりの日本語コメントが違反として報告される（＝正しいコードで赤くなる）。
        // 共有ヘルパーへ寄せたとき、閉じなかった場合を一律「行末まで」にして
        // <b>この誤検知を作り込んだ</b>ので、その退行をここで止める
        Assert.Empty(ScanLines(
            """"@{ var s = """ ; } @* Cache-Control はミドルウェアの既定に任せる *@""""));

        // <b>逐語的リテラルも改行をまたげる</b>ので、同じ扱いでなければならない。
        // ここで「引用符が 3 つ以上か」だけを数え直すと、引用符 1 つの @" が
        // 「またげない」側に落ちて行末まで飲み込まれ、後ろの @*…*@ が消えて赤くなる
        // （判定は CanSpanLines 1 か所。実測で、数え直す版はこの行だけが落ちる）
        Assert.Empty(ScanLines(
            """@{ var s = @"abc; } @* Cache-Control はミドルウェアの既定に任せる *@"""));

        // 同じく @""" は生文字列のフェンスではなく「逐語的リテラルの中の "" 」なので、
        // 終端を求める側と同じ答えにならなければならない
        Assert.Empty(ScanLines(
            """"@{ var s = @"""; } @* Cache-Control はミドルウェアの既定に任せる *@""""));
    ...
    private static (string Code, string? PendingCloser) StripComments(string line, string? pendingCloser)
    {
        ...
        while (i < line.Length)
        {
            ...
            if (line[i] == '"')
            {
                ...
                // 閉じなかったときの振る舞いは<b>この走査が行単位だから</b>自分で決める
                // (ソース全体を見る利用側は -1 を「読み取り不能」として扱えばよい)
                var end = closingQuote >= 0
                    // 閉じたなら、その次の位置まで
                    ? closingQuote + 1
                    // 閉じなかったときは、開きが生文字列のフェンスかどうかで分ける
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>改行をまたげるリテラルだけ別扱いにする理由。</b> この走査は<b>行単位</b>なので、
    /// 逐語的リテラル（<c>@"…"</c>）と生文字列（<c>"""…"""</c>）の開始行は必ず「閉じない」。
    /// そこで行末まで中身と見なすと、<b>同じ行の後ろに置かれた <c>@* … *@</c> が
    /// コメントとして落ちず、§5 どおりの日本語コメントで CI が赤くなる</b>
    /// （実測: <c>@{ var s = """ ; } @* Cache-Control は … *@</c> が違反として報告された）。
    /// その場合は開きの引用符 1 つだけを読み進め、残りはふつうに走査する。</para>
    ///
    /// <para><b>判定は <see cref="CSharpLiteral.CanSpanLines"/> に任せる。</b>
    /// ここで「引用符が 3 つ以上か」を数え直すと、<c>@"""</c>（逐語的リテラルの中の
    /// <c>""</c>）を生文字列のフェンスと取り違え、終端を求める側と<b>同じ入力に別の答え</b>を
    /// 出すことになる（§6 DRY）。</para>
    ///
    /// <para><b>またげないリテラルは行末まで中身と見なす。</b> 閉じない <c>"</c> の後ろに
    /// 実コードが続くことは無いので取りこぼしは起きず、中身を実コードとして貯めるぶん
    /// そこに書かれたキャッシュ指示は<b>見逃さない</b>（取りこぼす側へ倒れない）。</para>
    ///
    /// <para><b>残っている境界。</b> 改行をまたぐリテラルの<b>2 行目以降</b>は、
    /// リテラルの中だと分からないまま実コードとして走査される（この走査はリテラルの
    /// 状態を行をまたいで持たない。ブロックコメントだけが持ち越される）。
    /// 倒れる向きは<b>多く報告する側</b>なので見逃しにはならないが、
    /// 使い始めたらリテラルの持ち越しも足すこと。</para>
    /// </remarks>
    /// <param name="line">対象の行。</param>
    /// <param name="start">開きの二重引用符の位置。</param>
    /// <returns>次に読む位置。</returns>
    private static int UnterminatedLiteralEnd(string line, int start) =>
        // 改行をまたげるリテラルなら、開きの 1 つだけ読み進めて残りはふつうに走査する
        CSharpLiteral.CanSpanLines(line, start)
            ? start + 1
            // またげないリテラルは、行末までを中身と見なす
            : line.Length;
