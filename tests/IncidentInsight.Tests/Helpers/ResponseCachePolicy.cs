        ...
        Func<object, bool> matches)
    {
        ...
        foreach (var controller in controllers)
        {
            ...
            for (var type = controller; type is not null; type = type.BaseType)
            {
                ...
                // <b>実測(アプリ全体): 718 段・うち総称 55 段・クラス側の宣言 0 件。</b>
                // 省けるのは総称 55 段の再帰と正規表現で、残る 663 段はキャッシュ済みの
                // FullName を読むだけなので<b>効果は小さい</b> ——数字を大きく書かない
                // (この repo は実測の記述を根拠として扱うので、盛ると次の判断を誤らせる)。
