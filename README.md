...
代表フロー「報告 → なぜなぜ分析 → 再発防止策 → 登録完了（再発警告の自動表示）」:

![インシデント登録フローのデモ動画。3ステップウィザードで基本情報・なぜなぜ分析・再発防止策を入力して登録すると、類似インシデントの再発警告が自動表示される](docs/screenshots/incident-create-flow.gif)

### 主要画面

![インシデント一覧画面。部署・種別・重症度・対策状況で絞り込みでき、期限超過の行は赤色でハイライトされる](docs/screenshots/incident-list-default.png)

![インシデント新規登録フォーム。基本情報・原因分析・再発防止策の3ステップウィザード構成](docs/screenshots/incident-create-form.png)

![監査ログ一覧画面。すべての変更が自動記録され、PHI（個人情報）はマスキング処理済み](docs/screenshots/audit-log-list.png)

> スクリーンショット・デモはすべて開発用シードデータ（架空の人物名）によるものです。
> DB・常駐サーバーが必要なため公開デモ URL は設けず、デモ動画で代替しています（デプロイ手順は `docs/deployment.md` を参照）。

---

## なぜ作ったか

医療現場のインシデント（ヒヤリハット・事故）報告は、いまだに紙やExcel、メールで運用されている施設が少なくありません。
その結果、次のような課題が生まれます。

- **報告して終わりになる**: 原因分析や再発防止策の検討まで踏み込めず、同じ事故が繰り返される。
- **再発に気づけない**: 部署・種別・原因がバラバラに記録され、「似た事故が続いている」ことを誰も横断的に把握できない。
- **やりっぱなしになる**: 立てた対策が実際に効いたのか（有効性評価）を振り返る仕組みがない。
- **監査に耐えられない**: 「いつ・誰が・何を変更したか」の記録が残らず、医療安全の説明責任を果たせない。

IncidentInsight は、これらを **「報告 → なぜなぜ分析 → 再発防止策 → 完了 → 有効性評価 → 再発検知」という1本のライフサイクル** に乗せることで解決します。
同じ部署・種別・原因分類でインシデントが続けば自動で警告し、変更はすべて監査ログ（PHI はマスキング）に残ります。
単一クリニックの SQLite から、病院の SQL Server / クラウドの PostgreSQL まで、コードを変えずに同じ仕組みで運用できることを目指しています。
...
```mermaid
flowchart LR
    A[インシデント報告<br/>基本情報] --> B[なぜなぜ分析<br/>Why1〜Why5]
    B --> C[再発防止策<br/>計画 Planned]
    C --> D[進行中 InProgress]
    D --> E[完了 Completed]
    E --> F[有効性評価<br/>1〜5段階]
    F --> G{再発あり?}
    G -- いいえ --> H[クローズ]
    G -- はい --> I[警告バナー表示<br/>同部署・同種別・同原因分類で<br/>複数発生を検知]
    I --> B
```
...
- 原因カテゴリ 23 件のマスタデータ投入（全環境共通）
- サンプルインシデント 5 件（なぜなぜ分析・対策付き）の投入（開発環境 = Development のみ）
...
- **原因カテゴリ 23 件**（ヒューマンエラー/環境・設備/手順・マニュアル/教育・訓練/コミュニケーション の5大分類）— 全環境共通（`Seed`）
- **サンプルインシデント 5 件**（Why1〜Why5・再発防止策・有効性評価付き）— 開発環境（Development）のみ（`SeedDemoData`。架空の患者シナリオを本番 DB に混ぜないため）

シードデータはべき等設計（既存データがあればスキップ）のため、再起動で重複投入されません。
