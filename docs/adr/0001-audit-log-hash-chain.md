...
| `ConcurrencyToken` | `Guid`（`[ConcurrencyCheck]`） | 先端更新の直列化用。衝突した追記は巻き戻して再試行（決定性の肝①参照）。 |
...
代わりに、**監査行はその `RowHash` まで含めて 1 つのトランザクションで「完成形のまま」書き込む**。
業務変更（`Incident` / `CauseAnalysis` / `PreventiveMeasure`）の `SaveChanges` と
**同一トランザクション内**で、インターセプタが次を行う:

1. **チェーン先端を読み取り**: シングルトン行 `AuditChainState`（`LastSequence` と `LastRowHash` を保持）を取得。
2. **採番とハッシュ計算（挿入前）**: `Sequence = LastSequence + 1`、`PrevHash = LastRowHash` として
   式に従い `RowHash` を **挿入前に** 算出する。
3. **完成行の挿入 + 先端更新**: `RowHash` まで埋めた `AuditLog` 行を挿入し、`AuditChainState` の
   `LastSequence` / `LastRowHash` を更新する。
4. **コミット**: 業務変更・監査行・先端更新が **まとめて** コミットされる。途中状態は外から観測されない。

EF Core の単一 `SaveChanges` は既定でトランザクション境界を張るため、業務変更と監査行・先端更新を
**同一の `SaveChanges`（＝同一トランザクション）** に乗せることでこの原子性が成立する。
...
- **`DateTime` は `"o"`（round-trip, ISO 8601）固定**でシリアライズする。`ChangedAt` の `Kind`/精度差で
  バイトがブレないようにする（`IClock` が JST で供給する値をそのまま `"o"` 整形）。
...
- 検証ロジックは **純粋関数 `AuditChainVerifier`** として切り出す。`RecurrenceDetector.FindSimilar` と同じ方針で、
  **DB アクセスを持たず in-memory のリスト（`AuditLog` の昇順列）を受け取り、最初に壊れた `Id` を返す**。
  IO（DB 読み出し）は呼び出し側（コントローラ）が制御する。これにより InMemory でテスト可能にする
  （`RecurrenceServiceTests` / `RecurrenceDetectorTests` と同じテスト戦略）。
- 検証手順: 各行を `Id` 昇順に走査し、`PrevHash`（先頭はジェネシス値）と正準化バイト列から `RowHash` を再計算し、
  格納値と一致するか・前行の `RowHash` と当行の `PrevHash` が連結しているかを確認。
  **最初に不一致になった `Id` を報告**（どこから壊れたかを特定できる）。

### 管理者専用の検証アクション

- `AuditLogsController` に **管理者（`AppRoles.Admin`）専用**のチェーン検証アクションを追加する。
  全行を `Id` 昇順で読み出し（`SaveChanges` を介さない読み取り専用クエリ）、`AuditChainVerifier` に渡して
  「全件健全」または「最初に壊れた `Id`」を表示する。
...
- **provider 間で同一チェーン値になる正準化関数を 1 箇所に集約**する（`CanonicalAuditBytes(...)` のような単一関数）。
  `DateTime "o"` 整形・列順・UTF-8・null トークン・enum 文字列化（既存 `JsonStringEnumConverter`）を
...
- 導入時に既存行を `Id` 昇順でジェネシス封印（その時点のスナップショットとして 1 度だけチェーンを張る）するが、
...
- **Phase 1**: 列追加（`Sequence` / `PrevHash` / `RowHash`）＋ 新規 `AuditChainState` テーブル
  ＋ インターセプタのチェーン計算（業務変更と同一トランザクションでの**原子的・直列化書込**）
  ＋ 純粋関数 `AuditChainVerifier`（InMemory テスト）。
