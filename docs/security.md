...
- **例外**: 同ミドルウェアより手前で応答が完結する経路が 2 つあり、そこには上記のヘッダー群が
  ...
  1. 本番の HTTP → HTTPS リダイレクト(`UseHttpsRedirection` が返す 307)。本文を持たず、
     ...
     **なるなら削除してはいけません** —— 400 が止まるので直ったように見えて、実際には
     issue #64（Host ヘッダ偽装）へ移るだけです。この場合は実ホスト名へ書き換えてください。
     全許可になる経路は 2 つあり、**「生きた項目が残るかどうか」では判別できません**:
     項目が 0 件になって既定の `["*"]` へ落ちる形（`"   "` ・ `" ; "`）と、
     **残った項目自体がワイルドカードの形**（`"*; "` ・ `"incident.example.com;0.0.0.0; "`。
     後者は `ASPNETCORE_URLS` を写して書くと自然に生まれます）です。
