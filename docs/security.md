...
- **例外**: 同ミドルウェアより手前で応答が完結する経路が 2 つあり、そこには上記のヘッダー群が
  ...
  1. 本番の HTTP → HTTPS リダイレクト(`UseHttpsRedirection` が返す 307)。本文を持たず、
     ...
     **削除してよいかは「死んだ項目を消したあとの一覧が全許可になるか」で決まります。**
     ならないなら書き換えても削除してもかまいません
     （`AllowedHosts=incident.example.com; ${SECONDARY}` で `SECONDARY` が未定義のときは、
     書き換える先の実ホスト名が無いので末尾の `; ` を削除するのが唯一の直し方です）。
     **なるなら削除してはいけません** —— 実際には issue #64（Host ヘッダ偽装）が残るだけです。
     全許可になる経路は 2 つあり、**「生きた項目が残るかどうか」では判別できません**:
     1. **項目が 0 件になって既定の `["*"]` へ落ちる形**（`"   "` ・ `" ; "`）。
        それまで全リクエストが 400 だったのが止まるので直ったように見えます。
        実ホスト名へ書き換えてください。
     2. **残った項目自体がワイルドカードの形**（`"*; "` ・
        `"incident.example.com;0.0.0.0; "`。後者は `ASPNETCORE_URLS` を写すと自然に生まれます）。
        **この形はもともと全リクエストが 200 なので、症状の変化がありません** ——
        名指しされた空白の項目だけを直しても全許可のままです。
        **ワイルドカードの項目（`*` / `[::]` / `0.0.0.0`）自体を消して**、
        実ホスト名だけが残る状態にしてください。
     警告の文面がそのつど正しいほうを案内するので、そちらに従ってください。
