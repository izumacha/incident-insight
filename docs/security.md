...
- **例外**: 同ミドルウェアより手前で応答が完結する経路が 2 つあり、そこには上記のヘッダー群が
  ...
  1. 本番の HTTP → HTTPS リダイレクト(`UseHttpsRedirection` が返す 307)。本文を持たず、
     ...
     **一方、空白を挟むと逆に「一致しない項目」になります。** ホスト名の検査は項目を
     トリムしないため、前後に空白のある項目はどの `Host` とも一致しません
     (`" ; "` ・ `"  *  "` ・ `"   "` は実測でどれも 400)。
     ...
     **サイト全体は生きたまま、特定のホスト名だけが静かに落ちる**ので、
     ...
     （正規化できない綴りが含まれる）と変わるためです。**「死んだ項目しか無い＝すべて 400」
     とは限りません** —— `0.0.0.0<TAB>` のように正規化できない綴りだと、400 ではなく
     フレームワークが例外を投げます（500 相当）。
     **直し方は設定によって変わり、警告の文面がそのつど正しいほうを案内します。**
     生きた項目が 1 つでも残るなら、死んだ項目は書き換えても削除してもかまいません
     （`AllowedHosts=incident.example.com; ${SECONDARY}` で `SECONDARY` が未定義のときは、
     書き換える先の実ホスト名が無いので末尾の `; ` を削除するのが唯一の直し方です）。
     一方、**生きた項目が 1 つも無い場合は削除してはいけません** —— 項目が 0 件になると
     既定の `["*"]` へ落ちて**全ホストを受け付ける**状態になり、400 が止まるので直ったように
     見えて実際には issue #64（Host ヘッダ偽装）へ移るだけです。この場合は実ホスト名へ
     書き換えてください。
     なお、**2 本目の警告が 1 件も名指ししないこともあります**（`0.0<TAB>.0.0` のように
     項目の途中へ制御文字が紛れた形は前後の空白ではないため検出できません）。
     どちらの警告が出た場合も、**一覧の項目は 1 つずつ目視で確認してください。**
