...
- **例外**: 同ミドルウェアより手前で応答が完結する経路が 2 つあり、そこには上記のヘッダー群が
  ...
  1. 本番の HTTP → HTTPS リダイレクト(`UseHttpsRedirection` が返す 307)。本文を持たず、
     ...
     **サイト全体は生きたまま、特定のホスト名だけが静かに落ちる**ので、
     ...
     残りの項目しだいで、すべて 200（ワイルドカードが混ざっている）・すべて 400
     （死んだ項目しか無い）・その 1 件だけ 400（生きた項目と混在）・毎リクエスト例外
     （正規化できない綴り）と変わるためです。
     **直し方は「消す」ではなく「本当のホスト名に書き換える」です。** 消して項目が
     0 件になると既定の `["*"]` へ落ちて**全ホストを受け付ける**状態になります ——
     400 が止まるので直ったように見えますが、実際には issue #64（Host ヘッダ偽装）
     そのものへ移るだけです。警告が出たら**名指しされた項目を 1 つずつ書き換えてください。**
