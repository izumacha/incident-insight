...
- **例外**: 同ミドルウェアより手前で応答が完結する経路が 2 つあり、そこには上記のヘッダー群が
  ...
  1. 本番の HTTP → HTTPS リダイレクト(`UseHttpsRedirection` が返す 307)。本文を持たず、
     ...
     **サイト全体は生きたまま、特定のホスト名だけが静かに落ちる**ので、
     ...
     この警告は**その項目 1 件の話だけ**を述べ、一覧全体がどうなるかは名乗りません ——
     同じ一覧にワイルドカードがあれば結局すべて 200 になり、死んだ項目しか無ければ
     すべて 400 の全面障害になるので、残りの項目しだいで症状が変わるためです。
     **「書かれていないのと同じ」でもありません** —— 本当に消せば項目数が減り、
     0 件になれば既定の `["*"]` へ落ちて全許可になります（上記のとおり）。
     警告が出たら**一覧の項目を 1 つずつ確認してください。**
