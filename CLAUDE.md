...
**残っている境界**: issue #249 の形（開きと見なされないリテラルの中身に `//` がある行に、同じ行でキャッシュ指示を書く）は**ビューの中にだけ残る**。現時点でビューにその形は無く、§3 が「ページ固有のスクリプトを inline `<script>` に書かない」と定めているので現実味は低いが、**ビューの走査は依然として「増やしたことに気付く」ための網であって証明ではない**。ここに次の穴が出たら、綴りを足すのではなく Razor のパーサ（`Microsoft.AspNetCore.Razor.Language`）へ移すこと。
