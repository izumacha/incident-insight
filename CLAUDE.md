...
- **キャッシュ抑止は「誰も書かなければ `no-store`」の 1 規則で、指示を書く側が 3 つある** — `SecurityHeadersMiddleware` が応答開始直前に、キャッシュ指示の**無い**応答へ `Cache-Control: no-store` を入れる（応答の種類では振り分けない。CSV・添付画像のエクスポートを足しても既定で保護される）。**既に指示がある応答には一切触れない**のが要点で、それが静的アセットを `public,max-age=3600` のまま残す唯一の仕組み — `Program.cs` の `UseStaticFiles(OnPrepareResponse)` が自分で名乗ることで既定の対象から外れる。**この 2 ファイルは対になっており、片方だけを動かすと壊れる**（`OnPrepareResponse` を消すと css/js が毎回再取得になり、ミドルウェアの「譲る」挙動を消すと今度は静的アセットが `no-store` になる）。
  ...
  - **譲る相手の中身は問わないので、指示を書く側を全部塞ぐ必要がある。** PHI を返すアクションが「保存してよい」と名乗った瞬間、ミドルウェアは黙って譲る。書ける場所は 3 つあり、それぞれ別の検査が担当する:
    ...
    **Razor（`.cshtml`）は C# のパーサでは読めない**（マークアップの地の文に閉じない引用符が普通に現れ、C# として解析するとその 1 文字から先の解釈が総崩れになる）ので、ビューは別の走査が読む。**その走査は引用符を追わず、コメントは Razor の `@* *@` だけを見る。** Razor では HTML 属性の引用符と地の文の引用符を区別できないため、「ここはリテラルの開きか」は当てるしかなく、**当てると外れ方が両方向に出る**（当たらなければ中身が実コードとして解釈され、当たりすぎれば地の文が丸ごとリテラル扱いになる）。実測でも、直前の非空白文字から開きを当てていた版は `return "…"` / `=> "…"` の後ろを開きと見なさず、**中身の `//` で同じ行の書き込みが落ちていた**。C# の `//` と `/* */` を扱わないのも同じ理由で、こちらも両方向に壊れた — マークアップ中の裸の URL を行コメントと誤認して**同じ行の書き込みを落とし**、表示テキストの `incident_/*.csv` が閉じないコメントを開いて**正しいビューで走査が落ちた**。Razor のコメントは綴りが一意なので、`@* *@` だけならどちらも起きない。**姉妹の走査（`Views.ModelStateKeyPrefixMatchTests.Neutralize`）が同じ判断を同じ実測から先に下している** — ビューを読む 2 つの走査で答えが割れないよう、規則をそろえる（§6 DRY）。**ビューの `@{ }` に書いた `//` コメントが実コードとして読まれる**のが代償だが、倒れる向きは誤検知（気づける側）で、実測でもビューのコメントに走査対象の綴りを書いている箇所は無い。

    **読み終えた時点で `@*` が閉じていなければ落とす（fail-closed）。** 引用符を追わない以上、C# の文字列の中に書かれた `@*` もコメントの開始として読まれ、そこから下の行が走査から落ちる。Razor は地の文の `@` を `@@` と書かせるので、**コンパイルできるビューが閉じない `@*` で終わることは無い** — ここに来たのは走査が取り違えた証拠なので、黙って落とすより落ちるほうを取る（§9 fail-closed）。

    **残っている境界（issue #260）**: その fail-closed は**網であって証明ではない**。取り違えた `@*` を**後ろのどこかにある本物の `*@` が閉じてしまう**と何も言わず、飲み込まれた行は黙って走査から落ちる（実測で、`@functions { string A() => "x@*y"; }` の下に置いたキャッシュ指示が報告されなかった）。**§5 がビューに `@* *@` のコメントを求めている以上、閉じ綴りはたいていのビューに実在する。** 引用符を追う形へ戻しても解決しない（当てる以上、当たらない形が別の穴になる）ので、**閉じるには `Microsoft.AspNetCore.Razor.Language` でビューを読むしかない**。`.cs` を字句解析へ移したのと同じ手を、ビューにも打つこと。移すときは姉妹の走査と共有できるよう `Helpers/` へ置く（いま 2 つの走査が同じ近似を別々に持っている）。
