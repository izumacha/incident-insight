...
- **キャッシュ抑止は「誰も書かなければ `no-store`」の 1 規則で、指示を書く側が 3 つある** — `SecurityHeadersMiddleware` が応答開始直前に、キャッシュ指示の**無い**応答へ `Cache-Control: no-store` を入れる（応答の種類では振り分けない。CSV・添付画像のエクスポートを足しても既定で保護される）。**既に指示がある応答には一切触れない**のが要点で、それが静的アセットを `public,max-age=3600` のまま残す唯一の仕組み — `Program.cs` の `UseStaticFiles(OnPrepareResponse)` が自分で名乗ることで既定の対象から外れる。**この 2 ファイルは対になっており、片方だけを動かすと壊れる**（`OnPrepareResponse` を消すと css/js が毎回再取得になり、ミドルウェアの「譲る」挙動を消すと今度は静的アセットが `no-store` になる）。
  ...
  - **譲る相手の中身は問わないので、指示を書く側を全部塞ぐ必要がある。** PHI を返すアクションが「保存してよい」と名乗った瞬間、ミドルウェアは黙って譲る。書ける場所は 3 つあり、それぞれ別の検査が担当する:
    ...
    **Razor（`.cshtml`）は C# のパーサでは読めない**ので、近似の走査が残る — マークアップの地の文には閉じない引用符（`Bob's` / `"レベル3 以上`）が普通に現れ、C# として解析するとその 1 文字から先の解釈が総崩れになる。そこでは**単一引用符も二重引用符も「直前の非空白文字が `=` `(` `,` `[` `{` `:` `?` `+` `@` `$` のいずれか」かつ「その行の中で閉じているか」で判断する**。**2 つの手当てはどちらも実測の失敗から来ている**: 「閉じているか」だけだと `<p>Bob's</p> @* Cache-Control … *@ <a href='x'>` で `Bob's` の `'` が `href='` の `'` と閉じ、あいだのコメントが読まれず§5 どおりのコメントで CI が赤くなる。逆に「中身は 8 文字まで」と長さで縛ると `src='https://…'` がリテラルとして読めなくなり、中の `//` で**その行の残りが走査から丸ごと落ちた**（**誤検知を消すつもりで見逃しを作っていた**＝向きを間違えた手当ての実例）。**issue #252 の持ち越しはビュー側にも入れてある** — 改行をまたげる 2 種類（逐語的・生文字列）だけを次の行へ持ち越す（ふつうの `"…"` は改行をまたげないので持ち越さない。持ち越すと閉じ忘れの 1 行がファイルの残り全部を飲み込む）。**C# 側だけ直すと、同じ穴が `.cshtml` に残ったまま「直した」ことになる。**

    **ビューで扱うコメントは Razor の `@* *@` だけで、C# の `//` と `/* */` は扱わない。** Razor では文字列を最後まで追えない（HTML 属性の引用符と地の文の引用符を区別できない）ため、リテラルの外に見える `//` や `/*` をコメントの開始として読むと**両方向に壊れる**（どちらも実測）。**見逃す側**: マークアップ中の裸の URL（`詳しくは https://example.test を参照`）を行コメントと誤認し、同じ行に書かれたキャッシュ指示が落ちる。**誤検知側**: 表示テキストの `incident_/*.csv` が閉じないブロックコメントを開き、下の fail-closed が**存在しない原因を名指しして**落ちる（直し方も示せない）。Razor のコメントは綴りが一意なので、`@* *@` だけに絞ればどちらも起きない。**姉妹の走査（`Views.ModelStateKeyPrefixMatchTests.Neutralize`）が同じ判断を同じ実測から先に下している** — ビューを読む 2 つの走査で答えが割れないよう、規則をそろえる（§6 DRY）。代償は「ビューの `@{ }` の中に書いた `//` コメントが実コードとして読まれる」ことだが、倒れる向きは**誤検知（気づける側）**で、実測でもビューのコメントに走査対象の綴りを書いている箇所は無い。

    **持ち越しだけでは塞ぎ切れないので、「読めなかった」ことを fail-closed で落とす。** 持ち越しが効くのは**開きだと分かったリテラル**にだけで、ビューは開きを直前の文字から当てているため当たらない形がある（`return """…"""` / `=> "…"` の後ろ。実測で、その中身の `@*` が**以降のファイル全体を飲み込み**、下に書いたキャッシュ指示が黙って見逃された）。**開きの判定へ綴りを足す直し方は取らない**（地の文が開きと判定されて正しいコードで赤くなる向きへ踏み直すだけ）。代わりに、**ビューを読み終えた時点で `@*` が閉じていなければ落とす** — Razor は `@` の地の文を `@@` と書かせるので、コンパイルできるビューが閉じない `@*` で終わることは無く、誤って赤くなる心配が無い。直し方も 1 つに定まる（§3 のとおり、その C# をビューの外へ出す）。

    **残っている境界**: ビューの走査は依然として近似で、**開きの判定を直前の文字から当てている**（`return "…"` / `=> "…"` の後ろのリテラルは開きと見なされない）。いま塞いでいるのは「その中身に `@*` があってファイルを飲み込む」形だけで、`'` と `"` の取り違えが別の形で表に出る可能性は残る。**この走査は「増やしたことに気付く」ための網であって証明ではない。** ここに次の穴が出たら、綴りを足すのではなく Razor のパーサ（`Microsoft.AspNetCore.Razor.Language`）へ移すこと。
