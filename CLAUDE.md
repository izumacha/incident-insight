...
- **リバースプロキシ配下では `AllowedHosts` を実ホスト名に絞り、転送ヘッダを有効化**（issue #64）。既定 `"*"` のままにしない。**「絞れているか」の判定は `Models/Validation/AllowedHostsPolicy.IsPermissive` が正本で、`*` を 1 つでも含めば全許可と見なす** — `HostFiltering` は許可リストに `*` が 1 つでもあれば「空でない Host はすべて受け付ける」へ切り替わるため、`"*;incident.example.com"`（実ホスト名を「追加」しようとすると自然に書く形）は**絞れていない**。以前の判定は値全体が `"*"` と一致するかだけを見ていたのでこの綴りで警告が出ず、`docs/security.md` が案内している「警告ログが出ていないことを確認する」手順が**そのまま誤った安心**になっていた（実測で `*;real` が別ホストを 200 で受けることを確認済み）。`X-Forwarded-For` / `X-Forwarded-Proto` を信頼させるには `ForwardedHeaders:Enabled=true` ＋ `ForwardedHeaders:KnownProxies`。
