...
- **キャッシュ抑止は「誰も書かなければ `no-store`」の 1 規則で、指示を書く側が 3 つある** — `SecurityHeadersMiddleware` が応答開始直前に、キャッシュ指示の**無い**応答へ `Cache-Control: no-store` を入れる（応答の種類では振り分けない。CSV・添付画像のエクスポートを足しても既定で保護される）。**既に指示がある応答には一切触れない**のが要点で、それが静的アセットを `public,max-age=3600` のまま残す唯一の仕組み — `Program.cs` の `UseStaticFiles(OnPrepareResponse)` が自分で名乗ることで既定の対象から外れる。**この 2 ファイルは対になっており、片方だけを動かすと壊れる**（`OnPrepareResponse` を消すと css/js が毎回再取得になり、ミドルウェアの「譲る」挙動を消すと今度は静的アセットが `no-store` になる）。
  ...
  - **譲る相手の中身は問わないので、指示を書く側を全部塞ぐ必要がある。** PHI を返すアクションが「保存してよい」と名乗った瞬間、ミドルウェアは黙って譲る。書ける場所は 3 つあり、それぞれ別の検査が担当する:
    ...
    **持ち越しだけでは塞ぎ切れないので、「読めなかった」ことを fail-closed で落とす。** 持ち越しが効くのは**開きだと分かったリテラル**にだけで、ビューは開きを直前の文字から当てているため当たらない形がある（`return """…"""` / `=> "…"` の後ろ。実測で、その中身の `/*` が**以降のファイル全体を飲み込み**、下に書いたキャッシュ指示が黙って見逃された）。**開きの判定へ綴りを足す直し方は取らない**（地の文が開きと判定されて正しいコードで赤くなる向きへ踏み直すだけ）。代わりに、**ビューを読み終えた時点でブロックコメントが閉じていなければ落とす** — コンパイルできるソースがその状態で終わることは無いので、そこに来たのは走査が取り違えた証拠であり、誤って赤くなる心配が無い。直し方も 1 つに定まる（§3 のとおり、その C# をビューの外へ出す）。

    **残っている境界**: ビューでは、開きと見なされないリテラルの中身に `//` があると**その行の残り**が落ちる（issue #249 の形）。飲み込みが 1 行で収まるので上の fail-closed には掛からず、同じ行にキャッシュ指示を書いたときだけ見逃す。現時点でビューにその形は無く、§3 が「ページ固有のスクリプトを inline `<script>` に書かない」と定めているので現実味は低いが、**ビューの走査は依然として「増やしたことに気付く」ための網であって証明ではない**。ここに次の穴が出たら、綴りを足すのではなく Razor のパーサ（`Microsoft.AspNetCore.Razor.Language`）へ移すこと。
