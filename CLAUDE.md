...
- **リバースプロキシ配下では `AllowedHosts` を実ホスト名に絞り、転送ヘッダを有効化**（issue #64）。既定 `"*"` のままにしない。**「絞れているか」の判定は `Models/Validation/AllowedHostsPolicy.IsPermissive` が正本で、ワイルドカードを 1 つでも含めば全許可と見なす** — `HostFiltering.IsTopLevelWildcard` は **`*`（HTTP.sys）・`[::]`（IPv6 Any）・`0.0.0.0`（IPv4 Any）の 3 つ**を全許可として扱い、1 つでもあれば「空でない Host はすべて受け付ける」へ切り替わるため、`"*;incident.example.com"`（実ホスト名を「追加」しようとすると自然に書く形）は**絞れていない**。以前の判定は値全体が `"*"` と一致するかだけを見ていたのでこの綴りで警告が出ず、`docs/security.md` が案内している「警告ログが出ていないことを確認する」手順が**そのまま誤った安心**になっていた（実測で `*;real` と `0.0.0.0;real` がどちらも別ホストを 200 で受けることを確認済み）。**判定はホスト名の正規化（IDNA / NFKC）の後に行う** — フレームワークは各項目を `new HostString(entry).ToUriComponent()` に通してから 3 綴りと突き合わせるので、全角の `０.０.０.０` や読点の `0。0。0。0` も全許可になる（実測）。生の `Ordinal` 比較のままだとそこが「警告の出ない全許可」として残る。正規化できない綴りは判断できないので警告する側へ倒す（過剰に警告する＝安全側）。**「1 件も残らない値」も全許可**として扱う — 汎用ホストの既定設定は空の項目を落とした結果が 0 件なら `["*"]` へ落とすので、`;` / `;;` は全許可（実測で別ホストを 200 で受ける）。**だから判定はフレームワークの既定設定をそのまま写し、独自に丸めない** — とくに**先にトリムしない**こと。`" ; "` / `"  *  "` / `"   "` は項目が残るので既定へ落ちず、綴りも一致しないので**すべて 400（全拒否）**になる（実測）。トリムするとこれらが「全許可」に見え、**サイトが落ちるだけの設定を素通りと呼ぶ**ことになる — この警告が拾うべきなのは**黙って素通りする**形だけ（全拒否は運用者がすぐ気づく）。写した既定設定の 2 行と境界の実測値は `AllowedHostsPolicy` の docstring が正本で、ここに書き写さない。 フレームワーク側の前提（3 綴りとも全許可になること）は `HostFilteringShortCircuitTests` が、判定の境界は `AllowedHostsPolicyTests` が固定する。`X-Forwarded-For` / `X-Forwarded-Proto` を信頼させるには `ForwardedHeaders:Enabled=true` ＋ `ForwardedHeaders:KnownProxies`。
