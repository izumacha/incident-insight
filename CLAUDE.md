...
- **PHI を `AuditLog.ChangesJson` に平文で残さない** — 自由記述・個人名カラムには `[Sensitive(Mask.Redact)]` か `[Sensitive(Mask.Hash)]`（`Models/Auditing/SensitiveAttribute.cs`）を付与。インターセプタが `[REDACTED]` か HMAC-SHA256 擬似匿名化（鍵 = `Audit:HashSalt`）に置換。**新しい自由記述/個人名カラムは必ず annotate**。本番で `Audit:HashSalt` 空は起動失敗。salt ローテーションは過去ハッシュとの相関を失うので runbook に記録。
  ...
  - **「string 列」の判定は CLR の型ではなく「DB へ文字列として保存されるか」で行う** — `HasConversion` を通す列は CLR の型が enum のままなので、`ClrType == typeof(string)` だけで絞ると検出網から丸ごと外れる。しかも**変換後の型が現れる場所は書き方によって違う**（実測）: `HasConversion<string>()` は `GetProviderClrType()` に出て `GetValueConverter()` は `null`（`Severity` / `Status` / `MeasureType`）、`HasConversion(v => …, v => …)` は逆（`IncidentType`）。**片方だけを見ると、もう一方の書き方で保存される文字列列が丸ごと素通りする。** 将来 `HasConversion<string>()` で保存する自由記述の値オブジェクトを足しても分類と長さ上限が自動で追随するよう、`AuditedEntityModel.IsStoredAsString` が両方を見る。
  ...
  - **監査対象の一覧を読む側は必ず `AuditSaveChangesInterceptor.AuditedEntities` から導出する** — 現在の導出先は `AuditLogsController.AllowedEntityNames`（フィルタ許可リスト＋ドロップダウン）と 4 つのテスト（`AuditedEntityPhiClassificationTests` / `FreeTextMaxLengthAttributeTests` / `FieldLengthsTests` / `AuditEntityLabelCoverageTests`）。写しを持つと、監査対象を足したときにインターセプタだけが増えて写し側が取り残される。とくに `AllowedEntityNames` の取り残しは **fail-open**（`Contains` が false になって絞り込みが黙って外れ全件返る）＋**画面から到達不能**（ドロップダウンに出ない）の二重のずれを同時に起こす。`HashSet` の列挙順は保証されないので、導出時は序数順に並べ替えて表示順を固定する。
