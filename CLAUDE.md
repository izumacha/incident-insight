...
- **キャッシュ抑止は「誰も書かなければ `no-store`」の 1 規則で、指示を書く側が 3 つある** — `SecurityHeadersMiddleware` が応答開始直前に、キャッシュ指示の**無い**応答へ `Cache-Control: no-store` を入れる（応答の種類では振り分けない。CSV・添付画像のエクスポートを足しても既定で保護される）。**既に指示がある応答には一切触れない**のが要点で、それが静的アセットを `public,max-age=3600` のまま残す唯一の仕組み — `Program.cs` の `UseStaticFiles(OnPrepareResponse)` が自分で名乗ることで既定の対象から外れる。**この 2 ファイルは対になっており、片方だけを動かすと壊れる**（`OnPrepareResponse` を消すと css/js が毎回再取得になり、ミドルウェアの「譲る」挙動を消すと今度は静的アセットが `no-store` になる）。
  - **譲る相手の中身は問わないので、指示を書く側を全部塞ぐ必要がある。** PHI を返すアクションが「保存してよい」と名乗った瞬間、ミドルウェアは黙って譲る。書ける場所は 3 つあり、それぞれ別の検査が担当する:
    1. **`[ResponseCache]` の宣言** — `NoStore = true` が無い宣言は `ResponseCacheAttributePolicyTests` が落とす（`CacheProfileName` を使う形も、実際の指示が属性から読めないので fail-closed で落とす）。対象はアセンブリ上の具象 `ControllerBase` すべてから導出し、**抽象基底へ引き上げたアクション**も拾う（宣言元が自アセンブリかで切る。「その型が宣言したメソッドか」で切ると、基底は抽象なので走査対象に入らず具象では宣言元が基底なので弾かれ、**どこからも見えなくなる**）。名指しは継承して見えた型ではなく**実際に宣言している型**で行う（`override` を含む。そうしないと 1 つの宣言が派生の数だけ並び、名指しされたファイルには属性が無い）。
    ...
    3. **応答ヘッダーへの直接の書き込み**（`Response.Headers.CacheControl = "public,max-age=300";`）— 属性にも設定にも現れず、実行時の 1 文なのでリフレクションでは原理的に見えない。**手がかりを変えてソースの実在を見る**（`OnlyIntendedWriters_SetCacheControlDirectly`）。走査は **Web プロジェクト全体の `.cs` と `.cshtml`**で、**意図して書く 2 か所だけ**を理由付きの許可表（`IntendedCacheControlWriters`）に置く。**パスの形（`Controllers/` ・ `Views/`）で絞らない** — その外に置いた同じコード（`Pages/` のビュー、別フォルダの `partial class`）が素通りする（実測。`RepositoryPaths.EnumerateViewFiles` が同じ形の事故を記録している）。列挙は `.cs` と `.cshtml` の**両方**を回す（`EnumerateWebSourceFiles` は `*.cs` しか返さず、`Views/` 配下に `.cs` は 1 つも無いので、それだけでは「ビューも見る」条件が一度も成立しない＝実測で死んでいた）。照合は**大文字小文字を無視**する（`Response.Headers["cache-control"]` は実際に効く）。**コメントは取り除いてから判定**する — 行頭だけを見て捨てると、同じ行で閉じたコメントの後ろの実コード（`@* メモ *@ @{ … }`）が素通りする（実測）。逆にコメントで赤くもしない（§5 が求める日本語コメントで CI が落ちる検査は、いずれ検査ごと緩められる）。
  - **残っている境界**: 許可表は人が判断するエスケープハッチで、「その場所が本当に書いてよいか」は機械では決められない。**エントリが増える差分は、理由の妥当性をレビューで必ず確認する**（`LengthGovernanceExclusions` と同じ扱い）。表には「キーが実在すること」「理由が空でないこと」の 2 つの検査が掛かる。
