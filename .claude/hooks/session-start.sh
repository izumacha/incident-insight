...
if command -v dotnet >/dev/null 2>&1; then
  ...
  echo "dotnet は既にインストール済み: $(dotnet --version)"
else
  echo ".NET 8 SDK を apt からインストールします..."
  ...
  # インストール結果を確認のため表示する。
  echo "dotnet をインストールしました: $(dotnet --version)"
fi
...
if [ -f package.json ] && command -v npm >/dev/null 2>&1; then
  echo "npm 依存をインストールします..."
  npm install
fi

# --- .NET 依存の事前復元 -------------------------------------------------
# 起動直後の初回ビルドを速くするため NuGet パッケージを先に復元しておく。
echo "dotnet restore を実行します..."
dotnet restore
