...
///
/// <b>扱う単位は「文字数」だけ</b>。バイト長（<c>byte[]</c> の添付）やコレクションの件数を
/// 制限する項目は現時点でこのリポジトリに 1 つも無いため、その単位の定数もメッセージ書式も
/// 置いていない（CLAUDE.md §6「将来を見越した過度な抽象化を避ける」）。
/// 最初に別の単位の上限を足す人は、用途を表す定数と単位に合った日本語メッセージ書式を
/// ここへ追加し、あわせて長さ検査（<c>FieldLengthsTests</c>）の対象規則を拡張すること
/// ——検査はそう案内して落ちる（文字数用の <see cref="FreeText"/> / <see cref="MaxLengthMessage"/> を
/// 流用すると「添付は500文字以内で…」のような単位のずれた文言と上限になる）。
///
