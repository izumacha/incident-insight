...
///
/// <para><b>この画面だけ方式が「補完する」になる。</b> 一覧画面は条件を外せば
/// 「絞り込み無しの一覧」という正しい状態へ落ちるが、ダッシュボードには
/// <b>「期間なし」という状態が無い</b>(常に何らかの窓で集計する)ので、採用しなかった値は
/// 既定の期間へ補完するしかない。<b>方式が違っても伝え方は同じ</b> ——
/// 上の段落のとおり、方式(補完する / 採用しない)と伝え方(知らせる / 黙る)は別の軸。
...
/// 代わりに<b>この画面固有の手がかり</b>を 2 つ置いてある:
/// <c>DashboardPeriods_MatchThePeriodSwitcherInTheView</c> が許可リストと画面のボタンを
/// 双方向で突き合わせ、<c>DashboardPeriodFilter_GoesThroughTheResolver</c> が
/// 配線漏れを落とす。<b>3 画面目が許可リストの絞り込みを足したときに
