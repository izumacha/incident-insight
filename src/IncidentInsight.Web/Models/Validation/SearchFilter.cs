...
/// (判定は <c>Controllers.Internal.ListedValueFilterResolver</c>。方式は表の 2 択のうち
/// 「採用しない」——どちらも<b>コード側で閉じた語彙</b>なので、自由記述の発生部署のような
/// 「選択肢へ補完する」逃げ道は採れない)。</para>
///
/// <para><b>ダッシュボード(<c>/</c>)の集計期間も同じ扱い。</b>
/// <c>?period=</c> は <c>week</c> / <c>month</c> / <c>quarter</c> / <c>year</c> の
/// <b>コード側で閉じた語彙</b>(<c>ViewModels.DashboardViewModel.Periods</c>)で、
/// 打ち間違い・古いブックマークは既定の <c>year</c> へ丸められる。
/// 以前はその事実をどこにも出しておらず、<b>この表にも載っていなかった</b>
/// ——「表に載っていない画面が同じ入力を受けている」という、上の段落が穴だと
/// 名指ししている形そのもの(issue #204 課題 4 と同じ)。利用者は四半期のつもりで
/// 1 年分の KPI・トレンド・完了率を読み、期間切替ボタンは「1年」が選択中に見えるので
/// 食い違いにも気付けない。判定は <c>/AuditLogs</c> と同じ
/// <c>Controllers.Internal.ListedValueFilterResolver</c> を通す。</para>
///
/// <para><b>採用しなかったあとの扱いだけが、この画面では表の 2 択のどちらでもない。</b>
/// 一覧画面は条件を外せば「絞り込み無しの一覧」という正しい状態へ落ちるが、
/// ダッシュボードには<b>「期間なし」という状態が無い</b>(常に何らかの窓で集計する)ので、
/// 採用しなかった値は<b>既定の期間へ差し替える</b>しかない。
/// <b>これを「補完」と呼ばないこと</b> ——上の表で「補完」は<b>受け取った値を選択肢へ足して
/// 絞り込みを維持する</b>ことを指しており(発生部署・原因分類・担当部署がその方式)、
/// ここでやるのはその逆(受け取った値を捨てて既定へ置き換える)。同じ語で逆の操作を指すと、
/// 表を読んだ人がこの画面について反対の答えを得る。
/// <b>扱いが違っても伝え方は同じ</b> ——上の段落のとおり、
/// 採用しなかったあとの扱いと伝え方(知らせる / 黙る)は別の軸。
/// ただし<b>文面はそろえない</b>: 一覧画面の「選べる値ではない」の説明文は
/// 「その条件を外して一覧を表示しています」「下の『絞り込み』から選び直してください」と
/// 書いており、どちらもこの画面では事実と違う(外さずに既定へ丸めており、
/// 絞り込みパネルも無い)。<b>そろえるのは「同じことが起きた画面どうし」で、
/// 事実と違うことを言ってまで一字一句そろえるのではない</b>
/// (見出しが同じなら説明も同じ、を
/// <c>IgnoredFilterNotices_UseTheSameWordingAcrossScreens</c> が要求しているので、
/// 説明が違う以上は見出しも分ける)。</para>
///
/// <para><b>ここには署名から導く検出網が置けない。</b> <c>?period=</c> は <c>string?</c> で、
/// <c>string?</c> はどんな入力でも束縛できるため、<c>DateTime?</c> や enum のように
/// 「アプリ全体でその型の引数を受けるアクション」から対象を導けない
/// (<c>/AuditLogs</c> は画面の <c>&lt;select&gt;</c> の name を手がかりにしているが、
/// ダッシュボードの期間切替はリンクのボタン群でその手がかりも使えない)。
/// 代わりに<b>食い違いが構造的に作れない形</b>へ寄せてある:
/// 許可リストも画面のボタンも既定の表示名も <c>ViewModels.DashboardViewModel.PeriodChoices</c>
