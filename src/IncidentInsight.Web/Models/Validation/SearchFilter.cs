...
///
/// <para><b>残っている境界: <c>/AuditLogs</c> の中で答えが割れている(issue #220)。</b>
/// 上の段落が「<c>/AuditLogs</c> には注意書きの仕組みが無い」と書いていた前提は
/// issue #207 で消えた ——あの画面は<b>読めない期間指定については注意書きを出す</b>ようになった。
/// 一方でエンティティ名・操作種別が許可リストを外れたときは<b>いまも黙って落とす</b>ので、
/// 同じ画面の中で <c>?dateFrom=abc</c> は知らせ <c>?entityName=Bogus</c> は黙る、という
/// 状態になっている。**この 2 つは利用者から見た結果が区別できない**
/// (どちらも絞り込んだつもりで監査ログ全件が返る)し、どちらも古いブックマークか改ざんでしか
/// 起きない ——つまり上の段落が <c>/Incidents</c> の enum について
/// 「同じ画面の同じ入力欄で、綴りが <c>abc</c> なら知らせて <c>99</c> なら黙るのは
/// 一貫性の欠如そのもの」と書いているのと<b>まったく同じ形</b>が、画面をまたいで再現している。
/// <b>issue #207 で同時に直さなかった</b>のは、採用しなかった理由が違う
/// (「読めない」対「許可リストに無い」)ので旗も文面も別になり、
/// 「1 コミット = 1 論理変更」(CLAUDE.md §12)に収まらないから
/// ——<c>/Incidents</c> で同じ 2 つを issue #198 と #208 に分けたのと同じ切り方。
/// <b>この段落を消すときは、実際に手当てを入れたときにすること</b>
/// ——消しただけでは、割れたままの状態が表からも見えなくなる。</para>
///
