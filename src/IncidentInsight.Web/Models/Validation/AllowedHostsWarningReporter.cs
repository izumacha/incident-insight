    ...
    ILogger logger, string environmentName, Func<string?> readCurrentValue)
{
    ...
    public void ReportIfValueChanged()
    {
        ...
        // <b>代償は「待たされて困らない」ことではない（レビュー指摘。以前の記述は誤り）。</b>
        // 再読み込みの通知は OptionsMonitor が<b>登録順に同期で</b>呼ぶので、
        // この購読が先に登録されている分、ここで待つと
        // <b>その 1 回の通知にぶら下がる後続の処理がまとめて遅れる</b> ——
        // HostFilteringMiddleware 自身の再束縛だけでなく、同じ設定トークンに載っている
        // 他の設定の再束縛（レート制限・Identity・ログの絞り込みなど）も同じスレッドで
        // 順番待ちになる（レビュー指摘。以前の記述は影響先を 1 つに見積もっていた）——
        // 出力先が詰まる（ネットワーク出力のタイムアウト、止まったボリューム）と、
        // その間のリクエストは<b>古い許可リスト</b>で判定され続ける。
        // それでも錠へ入れるのは、外へ出すと記憶が実際の設定とずれ、
        // <b>次に本当に緩めたときに 1 本も出ない</b>（永続的な取りこぼし）ほうへ倒れるから。
        // 遅れは出力先のタイムアウトで頭打ちの一時的なもの、取りこぼしはプロセスの生涯
        // ——この比較で錠を選んでいる。<b>ここを広げる／外す人はこの比較をやり直すこと。</b>
        lock (_gate)
        {
            ...
            // <b>この代入の位置は不変条件ではない</b> ——retry を担っているのは旗なので、
            // 出す前へ動かしても挙動は<b>変わらない</b>。読み手が流れを追いやすいよう
            // 出し終えた後に置いてあるだけで、<b>守るべきは旗の側</b>
