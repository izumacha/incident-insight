...
{
    /// <inheritdoc />
    public partial class AddMaxLengthToApplicationUserProfileColumns : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // 本体が空なのは意図どおり。既定プロバイダの SQLite は TEXT 列に長さ制約を持たないため、
            // ApplicationUser.DisplayName / Department へ [MaxLength(100)] を付けても発行すべき DDL が無い。
            // それでもマイグレーションを起こすのは、モデルスナップショットに上限を記録して
            // 次回以降の差分計算を正しくするため(SQL Server / PostgreSQL 向けに Migrations/ を
            // 再生成したときは、この上限が nvarchar(100) / varchar(100) として反映される)。
        }
